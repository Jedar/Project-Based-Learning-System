...
@Component({
  selector: 'app-manager-student-create',
  templateUrl: './manager-student-create.component.html',
  styleUrls: ['./manager-student-create.component.css']
})
export class ManagerStudentCreateComponent implements OnInit {
  isUnique: any;
      ...
      this.authService.signUp(formValue["username"],md5Value,formValue["gender"],formValue["school"], 2).subscribe(
        result=>{
          if (result.code==200){ //服务器返回注册成功
            ...
            // this.cleanFormValue();
            this.router.navigateByUrl("/manager/student_list");
          }else{//服务器返回注册失败
            this.modal.error({
              nzTitle: "",
              nzContent: "创建失败"
