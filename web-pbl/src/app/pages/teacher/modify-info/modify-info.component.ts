...
export class ModifyInfoComponent implements OnInit {
  ...
  modifyInfo(): void {
    ...
    if (this.validateForm.valid && this.avatarUrl) {
      let md5Value = Md5.hashStr(formValue["password"]).toString();
      this.teacherService.modifyTeacherInfo(this.authService.getUserId(), formValue["username"], md5Value, formValue["gender"],this.avatarUrl)
        .subscribe(result => {
          if (result.code === 200) {
            this.modal.success({
              nzTitle: "",
